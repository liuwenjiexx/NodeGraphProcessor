    ...
    public static class NodeProvider
    {
        ...
        static void BuildScriptCache()
        {
            ...
            foreach (var nodeViewType in TypeCache.GetTypesDerivedFrom<BaseNodeView>())
            {
                ...
                {
                    var attrs = nodeViewType.GetCustomAttributes(typeof(NodeCustomEditor), false) as NodeCustomEditor[];

                    if (attrs != null && attrs.Length > 0)
                    {
                        Type nodeType = attrs.First().nodeType;
                        nodeViewPerType[nodeType] = nodeViewType;
                    }
                }
            }
        }
        ...
        static MonoScript AddNodeScriptAsset(Type type)
        {

            MonoScript nodeScriptAsset = null;

            if (IsNodeAccessibleFromMenu(type))
            {
                nodeScriptAsset = FindScriptFromClassName(type.Name);
            ...
            return nodeScriptAsset;
        }

        static MonoScript AddNodeViewScriptAsset(Type type)
        {
            MonoScript nodeViewScriptAsset = null;
            ...
            if (attrs != null && attrs.Length > 0)
            {
                ...
                nodeViewScriptAsset = FindScriptFromClassName(type.Name);
        ...
        {
            if (!nodeViewScripts.TryGetValue(type, out var script))
            {
                script = AddNodeViewScriptAsset(type);
            }
            return script;
        }
        ...
        {
            if (!nodeScripts.TryGetValue(type, out var script))
            {
                script = AddNodeScriptAsset(type);
            }
            return script;
        }
